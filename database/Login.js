...
class Login {
  static authenticate ({profile_name, password}) {
    const queryAuth = `SELECT user_id, region FROM user_profile WHERE profile_name = $1 AND password = $2`;
  ...
  static getVideoWatched({user_id}){
    const queryVideoWatched = `SELECT user_id video_id FROM video_watched WHERE user_id = $1`;
